...
client.on('messageReactionAdd', (messageReaction, user) => {
  let reactrole = JSON.parse(fs.readFileSync("./json/react.json", "utf8"));
  ...
  if (messageReaction.message.id === reactrole[messageReaction.message.guild.id].messageID) {
    ...
    member.roles.add(reactionrole)
    return user.send("**/" + user.username + "/DM** \n You have been given a role on " + messageReaction.message.guild.name + " from a reactrole.")
  }

  if (messageReaction.emoji.name === "🚩") {
    messageReaction.message.channel.send("**/" + messageReaction.message.guild.name + "/" + messageReaction.message.channel.name + "/** \n  " + "Flagged the post.").then(msg => {
      msg.delete(3000)
    })
...
client.on('messageReactionRemove', (messageReaction, user) => {
  ...
  if (messageReaction.message.id === reactrole[messageReaction.message.guild.id].messageID) {
    var reactionrole = reactrole[messageReaction.message.guild.id].role.id
    let member = messageReaction.message.guild.member(user);
    member.roles.remove(reactionrole)
    return user.send("**/" + user.username + "/DM** \n You have been removed from a role on " + messageReaction.message.guild.name + " from removing a reactrole.")
  }
})
...
client.on('message', async message => {
  ...
  let censor = JSON.parse(fs.readFileSync("./json/censor.json", "utf8"));
  if (!censor[message.guild.id]) {
    censor[message.guild.id] = {
      word: 0
    };
  }
  let whitelist = JSON.parse(fs.readFileSync("./json/whitelist.json", "utf8"));
  ...
  let watch = JSON.parse(fs.readFileSync("./json/watch.json", "utf8"));
  ...
  if (message.channel.id === watch[message.guild.id].toggle) {
    const watchmsg = message.guild.channels.cache.find(channel => channel.name === "terminal-logs");
    watchmsg.send("**/" + message.guild.name + "/" + message.channel.name + "/** \n  " + `There was a message in the watchlisted channel by ` + message.author.tag + ' (' + message.author.id + ') which contains: **' + message.content + '**')
  }

  let pg = JSON.parse(fs.readFileSync("./json/pg.json", "utf8"));
  ...
  let lockdown = JSON.parse(fs.readFileSync("./json/lockdown.json", "utf8"));
  ...
  let invites = JSON.parse(fs.readFileSync("./json/invites.json", "utf8"));
